...
public class ArticlePage {
	...
	// Get text from the main article headline
	public String headlineText() {
		try {
			return h1Headline.getText().trim();
		} catch (Exception e) {
			return "";
		}
	}

	// Check if the headline is visible and not empty
	public boolean isHeadlineVisible() {
		try {
			return h1Headline.isDisplayed() && !headlineText().isEmpty();
		} catch (Exception e) {
			return false;
		}
	}

	// Check if a publish or updated date is visible
	...
	// Check if either author name or section/category label is visible
	...
	// Check if the article has multiple readable paragraphs
	...
	// Check if images are loaded and contain non-empty alt text
	...
	// Check if canonical link exists and is absolute (starts with http)
	...
	// Check if social meta tags are present
	public boolean hasSocialMetas() {
		return socialMetas != null && socialMetas.size() >= 2;
	}

	// Check if the JSON-LD data contains NewsArticle schema with headline and
	// datePublished
		...
		for (WebElement block : jsonLdBlocks) {
			try {
				...
				if (json != null && json.contains("\"@type\"") && json.contains("NewsArticle")) {
					...
					if (hasHeadline && hasDatePublished)
						return true;
				}
			} catch (Exception ignore) {
			}
	...
	public String googleNewsFollowHref() {
		try {
			return googleNewsLinks.stream().filter(WebElement::isDisplayed).map(el -> el.getAttribute("href"))
					.filter(href -> href != null && !href.isEmpty()).findFirst().orElse(null);
		} catch (Exception e) {
			return null;
		}
	}
	...
	// Check the current page is a Google News or consent page
	...
	// Click on the Google News follow link and verify if it opens a valid page
	public boolean openGoogleNewsFollowAndVerify() {
		try {
			// find the first visible Google News link
			...
			// If on consent page — click "Accept all"
